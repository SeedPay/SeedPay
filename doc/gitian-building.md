...
*Setup instructions for a gitian build of Seedpay using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Seedpay
...
- [Building Seedpay](#building-Seedpay)
...
Clone the git repositories for Seedpay and gitian and then checkout the Seedpay version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Seedpay-crypto/Seedpay.git
cd Seedpay
...
Follow the instructions in [doc/release-process.md](release-process.md) in the Seedpay repository
...
To build Seedpay (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the Seedpay repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Seedpay/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/Seedpay-crypto/Seedpay
...
For example:
```bash
URL=https://github.com/crowning-/Seedpay.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit Seedpay=${COMMIT} --url Seedpay=${URL} ../Seedpay/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Seedpay=${COMMIT} --url Seedpay=${URL} ../Seedpay/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Seedpay=${COMMIT} --url Seedpay=${URL} ../Seedpay/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Seedpay-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Seedpay-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/Seedpay-build.assert
```
...
[Seedpay/gitian.sigs](https://github.com/Seedpay-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
